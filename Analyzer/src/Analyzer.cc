...
Analyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	cout<<"Run: "<<RunNumber<<"     LumiBlock: "<< LumiNumber<<"     Event: "<< EventNumber<<endl;
	nevents++;
   ...
   if(runHLT_==1){
	...
	if(runcosmicmuons_)
	{
		...
		edm::Handle<edm::View<pat::Muon> > muonHandle;
		iEvent.getByLabel(muoLabel_,muonHandle);
		vector <pat::Muon> mycosmicmuon_container;
		CosmicMuon_n = muonHandle->size();
		const edm::View<pat::Muon> & muons = *muonHandle;   // const ... &, we don't make a copy of it!
		for(edm::View<pat::Muon>::const_iterator muon = muons.begin(); muon!=muons.end(); ++muon){
			//if(muon->isGlobalMuon())
			mycosmicmuon_container.push_back(*muon);
		}
		 */
		//cosmic muon
				...
				if(runHErechit_ && pho_isEB[x]){
					...
					for(HBHERecHitCollection::const_iterator hh = hbhe->begin(); hh != hbhe->end() && HERecHit_subset_n<10000; hh++){
						...
						if (id.subdet()==2){
							...
							}//if delta R from photon is small try to save
						}
					}
				}
